...
impl BlockExplorer {
            ...
            if let Ok(block) = blockchain.get_block_by_height(height) {
                for tx in &block.transactions {
                    ...
                    if !addresses.iter().any(|a: &AddressSummary| a.address == tx.sender) {
                        if let Ok(balance) = blockchain.get_balance(&tx.sender) {
                    ...
                    if !addresses.iter().any(|a: &AddressSummary| a.address == tx.receiver) {
                        if let Ok(balance) = blockchain.get_balance(&tx.receiver) {
                            addresses.push(AddressSummary {
    ...
    pub async fn get_network_health(&self) -> Result<NetworkHealth> {
        ...
        for height in 1..=current_height {
            if let (Ok(current_block), Ok(prev_block)) = (
                blockchain.get_block_by_height(height),
                blockchain.get_block_by_height(height - 1)
            ) {
        ...
        let last_block_time = if let Ok(last_block) = blockchain.get_block_by_height(current_height) {
        ...
        Ok(NetworkHealth {
            status,
            block_time_variance,
            network_difficulty: blockchain.get_difficulty() as f64,
    ...
    pub async fn get_statistics(&self) -> Result<ExplorerStatistics> {
        ...
        let mut active_addresses_24h = 0;
        ...
        total_addresses = seen_addresses.len() as u64;
        active_addresses_24h = active_addresses.len() as u64;
        
        // Count contracts (simplified)
        for address in &seen_addresses {
            if blockchain.is_contract_address(address).unwrap_or(false) {
                total_contracts += 1;
            }
        }
    ...
    async fn perform_search(&self, query: &str) -> Result<SearchResult> {
        ...
        if let Ok(height) = query.parse::<u64>() {
            if blockchain.get_block_by_height(height).is_ok() {
        ...
        if blockchain.get_block_by_hash(query).is_ok() {
            let block_details = self.get_block_details(query).await?;
            return Ok(SearchResult {
                result_type: SearchResultType::Block,
                data: serde_json::to_value(block_details)?,
        ...
        if blockchain.get_transaction_by_hash(query).is_ok() {
        ...
        if blockchain.get_balance(query).is_ok() {
